...
std::vector<std::string> cls_STEPfile::ListByFilter(std::string p_filter)
{
   ...
   //// Цикл по всем найденным сущностям.
...
void cls_STEPfile::FillTreeWidget(QTreeWidget* p_treeWidget)
{
    //// Link if not yet linked
    if (!mLinked) this->Link();

    mEntityInstanceList->FillTreeWidget(p_treeWidget);
}

void cls_STEPfile::FillTreeWidgetBREP(QTreeWidget* p_treeWidget)
{
    //// Link if not yet linked
    if (!mLinked) this->Link();

    //// Generate and fill BREP links in not yet
    if (!mBREPfilled) this->GenerateAndFillBrepLinks();

    //mEntityInstanceList->FillTreeWidgetBREP(p_treeWidget);
    ...
    for (iterSolids = foundList.begin(); iterSolids != foundList.end(); ++iterSolids)
    {
        //TODO this shit will not work for CEI
        cls_SEI* curFilteredSEI;
        curFilteredSEI = static_cast<cls_SEI*> (iterSolids->second);

        QTreeWidgetItem* curSolidItem = new QTreeWidgetItem(p_treeWidget);
        curSolidItem->setText(0, QString::fromStdString(curFilteredSEI->GetName()));

        curFilteredSEI->FillTreeWidgetBREP(curSolidItem);
