...
{
    //// Loop over the list and call Dump() for each element
    std::map<unsigned int, cls_EI*>::const_iterator iter;
    for (iter = mEIs.begin(); iter != mEIs.end(); ++iter)
    {
        //// iter->first is the entity ID,
        //// iter->second - pointer to the entity instance object.
        (iter->second)->Dump();
    }
}
...
{
    //// Loop over the list and call Link() for each element
    std::map<unsigned int, cls_EI*>::const_iterator iter;
    for (iter = mEIs.begin(); iter != mEIs.end(); ++iter)
    {
        //// iter->first is the entity ID,
...
void cls_EIlist::FillTreeWidget(QTreeWidget* p_treeWidget)
{
    cls_SEI* v_curSEI;
    cls_CEI* v_curCEI;
    ...
    {
        QTreeWidgetItem* curItem = new QTreeWidgetItem(p_treeWidget);
        QString v_name = "#" + QString::number((v_iter->second)->GetN()) + "=";
        switch ((v_iter->second)->GetType())
        {
        case etnSIMPLE:
            v_curSEI = static_cast<cls_SEI*> (v_iter->second);
            v_name.append(QString::fromStdString(v_curSEI->GetName()));
            curItem->setText(0, v_name);

            //// Go deeper
            v_curSEI->FillTreeWidget(curItem);

            break;
        case etnCOMPLEX:
            v_curCEI = static_cast<cls_CEI*> (v_iter->second);
            v_name.append("COMPLEX EI");
            curItem->setText(0, v_name);

            //// Go deeper
            v_curCEI->FillTreeWidget(curItem);
...
void cls_EIlist::FillTreeWidgetBREP(QTreeWidget* p_treeWidget)
{
    cls_SEI* v_curSEI;
    cls_CEI* v_curCEI;
    std::map<unsigned int, cls_EI*>::const_iterator v_iter;
